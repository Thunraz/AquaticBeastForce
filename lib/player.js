...
function Player(x, y, game) {
    ...
    this.game.backgroundLayer.push(this.shadow);
}
...
Player.prototype.createTiles = function(x, y) {
    ...
    for(var i = 0; i <= (this.game.canvas.width / this.game.camera.scale / 16) + 1; i++) {
        for(var j = 0; j <= (this.game.canvas.height / this.game.camera.scale / 16) + 1; j++) {
            var gx = Math.floor((x + i * 16) / 16) * 16;
            var gy = Math.floor((y + j * 16) / 16) * 16;

            if((random(gx) * random(gy)) > 0.5) {
                new GroundTile(gx, gy, Math.round(Math.random() * 3), this.game);
            }
        }
    }
};
