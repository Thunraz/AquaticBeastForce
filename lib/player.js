...
Player.prototype.createTiles = function(x, y) {
    x = Math.floor(x / 16) * 16 - 80;
    y = Math.floor(y / 16) * 16 - 40;

    console.log(x + ' ' + y);

    for(var i = 0; i < (canvas.width / camera.scale / 16); i++) {
        for(var j = 0; j < (canvas.height / camera.scale / 16); j++) {
            var gx = Math.floor((x + i * 16) / 16) * 16;
            var gy = Math.floor((y + j * 16) / 16) * 16;
            if((random(gx) * random(gy)) > 0.5) {
                new GroundTile(gx, gy);
            }
        }
    }
};
